...
class RedisDataAccessor {
  ...
  dataset2Vectors(dataset, keyFn) { // dataset 转为向量 [key1,json1,key2,json2]
  ...
  async loadDataSet(dataset) { // dataset 为 user-item 格式
    ...
    let userKeyValues = this.dataset2Vectors(dataset, userVectorKey);
    let itemKeyValues = this.dataset2Vectors(reversedDataset, itemVectorKey);
